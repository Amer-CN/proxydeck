...
## 攒版发布（2026-09-13 用户裁决，并行会话必须遵守）

用户原话：「我暂时还不想发版，因为并行会话还有几个在更新别的东西，比如增加其他功能、
优化修复等。所以我想把这些全部搞完之后，再重新发新版本，不要一会儿又发版，一会儿又发版。」

**发版节奏由用户统一掌握，不再改完一个功能就发一版。**

并行会话纪律（每个会话各自遵守，不需要互相协调）：
- **只提交代码，不发版**：改完 → 跑测试 → `git commit` → 停。**不升版本号、不动
  `CHANGELOG.md`、不打 tag、不做 GitHub Release、不跑 `python build.py` 覆盖
  `ProxyDeck.exe`**（那五处升版本清单见上文，全部属于发版动作）。
- **不自行换装运行中的服务**：换装要断服务，须用户确认（见「运行服务管理规矩」）。
  测试构建用临时名（`go build -o 临时名.exe ./app`）或测试端口（`--port 18788`），
  不碰 8788。
- **改完在提交信息里写清「属哪一版待办」**，方便发版时汇总。
- 用户说「发版」时，才由那个会话统一执行：重估段位（按最高档现算）→ 升版本号
  → 补 CHANGELOG 顶条 → 构建 → 打 tag → 发 Release（附 exe 附件）。

**发版积压（发版时逐条核对，改完划掉）**：
1. ~~种子卫兵双轨道扫描（`d0fd5c1`，2026-09-13）——npm + 桌面端两条版本轨道都盯，
...
- **hy4-preview 限流兜底（v3.8.1）**：撞配额 429 自动切 fallback（缺省
  ...
  POST 响应同构带 hy4_limit、设置甲板拨杆行带三态状态字「未限流/已限流跑
  fallback/关」、警告条随 fallback 配置显示不再写死；v3.14.0 起点阵屏的
  「hy4 兜底」状态行已删（纯展示无功能，用户裁决）。实测教训：
  上游限流是滑动窗口+按日配额双形态
...
## 当前状态（2026-09-13，v3.14.0 已发布·攒版继续）
...
- **v3.15.4 已发布（2026-09-14）**：修彻底关机漏停（`63f5b3c`）——拨杆停堆清单
  硬编码只写早期 3 个插件，后加的 bai/comate/qoder 永远停不掉（用户实测踩中：
  关机后 3 后端存活）。改为从 PluginList（Go 侧注册表）现取全量 + 失败兜底显式
  六项，新增插件从此不漂移。段位=修订号。发版链 `9df2771` → `d73a646`（exe，
  干净树 @ 9df2771，sha256 8c70f3c5…）；tag + Release 带 exe 已核验。同日四连发
  v3.15.1/2/3/4，均为修订号。旧包 `.work/ProxyDeck.prev-v3152.exe` 及更早可清。

- **v3.15.3 已发布（2026-09-14）**：修更新检查跨发版失明（`c70500d`+`d030fdd`）——
  原成功缓存永久有效 + 拉取 goroutine 一次性，GUI 开着跨发版本会话永远误报
  「已是最新」（用户实测踩中）。改为成功缓存 30 分钟保鲜 + 过期踢常驻拉取循环
  （断网限频 30s）。段位=修订号。发版链 `c70500d` → `d030fdd`（exe，干净树 @
  c70500d，sha256 cf35286e…）；tag + Release 带 exe 已核验。本地已腾位换装
  （旧包 `.work/ProxyDeck.prev-v3152.exe`）。同日连发 v3.15.1/2/3 三版，均为修订号。

- **v3.15.2 已发布（2026-09-14）**：更新下载提速——一键更新镜像链重排
  （`c4ae6f4`，gh-proxy.com 置前；ghfast.top 无梯子直连已死、降为第二候选）。
  段位=修订号。发版链 `52bf5ec`（版本串 8 处+CHANGELOG）→ `8bf6cf2`（exe，干净树 @
  52bf5ec，sha256 55a51ddc…）；tag + Release 带 exe 附件已核验。本地已腾位换装
  （旧包 `.work/ProxyDeck.prev-v3151.exe`）。镜像实测方法论：curl `--noproxy "*"` +
  `--max-time` 逐源测（ghfast 直连 TCP 超时、gh-proxy 12.5MB/15s、直连 GitHub 无
  梯子 TLS 被掐）——gh-fast 类公共镜像随时会死，发版前顺手测一遍首选源。

- **v3.15.1 已发布（2026-09-14）**：全链路性能优化两批一次带上（`0390e15` +
  `2343b25`，本会话独立完成）：UI 线程阻塞绑定清零（点火 15s/探活 10s/测延迟
  分钟级全部后台化）、轮询去重门控与内容键防重建、隐藏窗停摆、里程表就地更新、
  tuanjie token 指纹缓存、stats 节流落盘。段位=修订号（无新功能无界面变化）。
  发版链 `477007c`（版本串 8 处+CHANGELOG）→ `d4de4a9`（exe，worktree 干净树 @
  477007c，sha256 cead2d70…）；推送 + tag + Release v3.15.1 带 exe 附件已核验。
  本地已腾位换装（旧包存 `.work/ProxyDeck.prev-v3150.exe`），**GUI 重启一次即用新版**，
  插件后端不用动（想让后端换包须熄火再点火）。

- **v3.14.0 已发布（2026-09-13）**：攒版内容一次带上——seedguard 桌面端双轨道
  （`d0fd5c1`，并行会话）+ 全量测试修复批（`d9fda75`：团结单账号 402 熔断/设置甲板
  i18n 七语/切甲板滚动回顶/自检层 pointer-events/重拉去抖/origLabel 缓存修复）+
  `c16d086`（账号池拨杆键盘重拉）+ `0087ab2`（删 hy4 兜底行/脱敏展示行）。
  发版链 `1b85281`（版本串 8 处+CHANGELOG）→ `d97c304`（exe，worktree 干净树 @
  1b85281，官方旗子）；推送 `7f4a2c5..d97c304`；Release v3.14.0 带 exe 附件已核验。
  段位=次版本（攒版含新功能，用户裁决 v3.14.0）；CHANGELOG 5 行与浮层逐字同源。
- **c16d086（已随 v3.14.0 发布）**：账号池区补拨杆 keydown 重拉（键盘路径走引擎
  gearSelect 不派发 click，故与 click 并列监听，引擎代码未动）；连按去抖后补于
  `d9fda75`。
...
- 远程 main 与本地同步；仓库 github.com/Amer-CN/proxydeck（remote 名 `myrepo`，
  ...
  GitHub 会重定向，能用但名字陈旧）
- 服务现状（2026-09-13 13:5x 复核）：用户经「彻底关机」拨杆全停后自行重跑根目录
  **v3.14.0 新包**并逐个拉杆。GUI PID 28816（本地端口 13679）；六后端 200：
  8785/8786/8787/8788/8789/8891；55990 按需未起。测试遗留的注水浮窗进程（36464）
  已清，现 7 进程 = GUI + 六后端。旧包回滚副本 `.work/ProxyDeck.prev-v3131.exe`
  稳定跑几天后删。⚠ **拉杆不会换包**：`pluginStart` 探活到端口健康就「接管复用，不杀不重启」（见下条），
  想让后端换包必须**熄火再点火**。
...
- **已知遗留（2026-09-13 全量测试实测；①②③ 已修待攒版，④ 待办，⑤ 已销账）**：
  ① `api-key.txt` 存储的 CommandCode key 已失效（55990 三协议实测全 401），待用户
  换 key；候选改进：GUI 对存储 key 做探活提示。**已修待攒版**（2026-09-13 本次会话）：
  `internal/proxy/probe.go` 新增 `ProbeKey`（打 whoami，免费端点），`app/bridge.go`
  新增 `ccProbeKey` 绑定（只回状态字，不回 key 与响应体），`app/ui.html` 三处触发
  （初始化 / 保存 key 后 / 30 分钟周期），状态不变静默、401 才 ERR+toast、恢复写 OK。
  用户换新 key 后即自愈，无需再动代码。
  ② 8787 上游不支持对象式 `tool_choice`（强制工具 400 Unmarshal failed；字符串式
  auto/required 正常，8788 两种都支持）——候选修复：代理层降级转发。**已修待攒版**
  （2026-09-13 本次会话）：`internal/codebuddy/server.go` 新增 `normalizeToolChoice`，
  白名单拷贝后归一——`type` 为 auto/none/required 取该字符串、强制指定式降级
  `"required"`（保住必须调工具语义，模型自选函数）、字符串原样、无 tools 则删除；
  单测 `internal/codebuddy/server_test.go`（4 例）。
  ③ 团结甲板「积分消耗 TOP」每 ~3s 刷一条 NET 日志，冲刷有效事件——候选降噪。
  **已修待攒版**（2026-09-13 本次会话）：`app/ui.html` renderTuanjieQuota 内恢复
  v2.5.4（`97680fc`）的内容键防抖 `V.lastTopKey`（来源+积分），同内容不写日志；
  与渲染层 `lastBarsKey` 独立。
  ④ 低优攒办：错误响应格式不统一（404/405/双重包装）、插件端口无鉴权（127.0.0.1
  缓解）、Comate usage 恒 0、账号区无障碍标注缺失。
  ⑤ 点火拉杆「拉回熄火」在自动化环境 4 次未触发（疑似前台竞争假象），**人工未复核**
  ——本次用户走的是「彻底关机」路径，不经拉杆回拉；下次手动拉一次即可定谳。
  **已销账（2026-09-13 用户人工拉杆确认可用）**：此前自动化未触发是前台竞争假象，
  非产品 bug，代码不动。
  测试全记录：2026-09-13 三轮全量（后端 API→深挖→GUI 副页），tool_calls/流式终止/
  媒体改路由/并发选号/三协议错误封装/更新检查均实测通过
...
- 实测记录（2026-09-06 两轮全量 + 2026-09-13 三轮全量）：结论已并入上文模型情报、
  坑 14/15/16 与「已知遗留」；唯一未覆盖：B.AI 6-8M 字符级 WAF 极端量级（判定
  不值得复现）
- 攒版节奏继续有效（2026-09-13 用户裁决）：各会话只提交不发版，用户说「发版」时
  由该会话统一升版本号（段位现算）、worktree 干净树构建并附 exe。新坑/结论随时
  按惯例入册
