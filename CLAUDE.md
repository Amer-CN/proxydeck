...
## 当前状态（2026-09-12，v3.13.0 已发版）

- **v3.13.0 已发布**：账号矩阵三批——① 积分按账号切换（`/quota?uid=`、缓存按区+号分键、
...
- 远程 main 与本地同步；仓库 github.com/Amer-CN/proxydeck（remote 名 `myrepo`，
  ...
  GitHub 会重定向，能用但名字陈旧）
- 服务现状（2026-09-12 21:0x 复核）：GUI（PID 15724，21:03 起）已是 **v3.13.0 新包**
  （单 GUI，残留旧 GUI 已清理）。六后端在线：8787/8788/8789 是用户 16:29 拉杆后的
  新映像（账号池已在 8787 实测生效）；8785/8786/8891 仍是旧映像进程（无在途功能
  未换包）。⚠ **下次换装这两条不用再拉**：8787/8789 当前映像已含全部池功能。
