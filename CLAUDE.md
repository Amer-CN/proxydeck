...
## 当前状态（2026-09-12，v3.11.0 已发版）

- **v3.11.0 已发布**：脱敏扩全角色（根治 11128 自我投毒）+ 历史版本日志全量改大白话。
  提交 `ed45d97`（发版）+ `78a1ae4`（exe，干净树 @ ed45d97）。
  段位校正：v3.10.8 含新功能却按修订号发，本版回到次版本段位（3.10.8 → 3.11.0）
- **v3.10.8**：WorkBuddy 国际版接入——同一甲板内「国内版 ⇄ 国际版」切换
  （不新增甲板/模式键），国际版走 www.codebuddy.ai（8789），模型池 15 个（3 免费 +
  12 付费，逐条实测）；出站角色降级 developer→system（修 harness 一律 11128）。
  v3.10.7（账号池浮窗补全）、v3.10.6（思考链路）及更早细节见 `CHANGELOG.md`
- 远程 main 与本地同步；仓库 github.com/Amer-CN/proxydeck（remote 名 `myrepo`，
  ...
  GitHub 会重定向，能用但名字陈旧）
- 服务现状（2026-09-12 02:25 复核）：8785 / 8786 / 8787 / 8788 / 8789 / 8891 六个插件
  **全部在线**；GUI（PID 7540，02:21:38 起）已是 v3.11.0 新包。
  **但插件进程都早于 02:21:09 的换装时点**（8787/8789 起于 01:59，其余起于 09-11），
  Windows 下进程持有的是换装前的映像 → **后端仍跑旧包**（8787/8789 跑的是 01:58 那份
  含全角色脱敏修复的包，功能已是修好的；版本串仍自报 v3.10.8）。
  ⚠ **拉杆不会换包**：`pluginStart` 探活到端口健康就「接管复用，不杀不重启」（见下条），
  所以想让某个后端也换成 v3.11.0，必须**熄火再点火**（拉回最左 → 再拉到最右），
  只拉一次只是接管。后端版本串对用户不可见，功能修复已在线，故不强制。
...
- 下一版待办：暂无（v3.11.0 已发版）；新坑/结论随时按惯例入册
