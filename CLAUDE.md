...
- **待发版积压**：见上文「攒版发布」节（种子卫兵双轨道扫描 `d0fd5c1` 等）。
- **c16d086 已提交未发版**：账号池区补拨杆 keydown 重拉（用户明确要求补①；键盘路径走引擎
  gearSelect 不派发 click，故与 click 并列监听，引擎代码未动）。仅 1 文件
  （app/ui.html +12/-2），随下个正式版一起发（**版本号发版时现算**，原先预写的
  v3.13.2 已作废）。
...
- 发版积压清单见上文「攒版发布」节（2026-09-13 起发版节奏由用户统一掌握，
  各会话只提交不发版；原先此处的「下一版待办」已并入该节，不再单独维护）。
  新坑/结论随时按惯例入册
