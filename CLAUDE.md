...
- 远程 main 与本地同步；仓库 github.com/Amer-CN/proxydeck（remote 名 `myrepo`，
  ...
  GitHub 会重定向，能用但名字陈旧）
- 服务现状（2026-09-12 02:21 复核）：8785 / 8786 / 8787 / 8788 / 8789 / 8891 六个插件
  **全部在线，都是 v3.11.0 新 exe**（腾位法换装，后端全程未断）；55990 主代理未起
  （按需点火）。**GUI 不持久化插件启动状态**（`plugins.go` 无写盘），**重启 GUI 不会
  自动点火、需逐个拉杆**——跨会话常驻只能 GUI 点火（坑 4）
