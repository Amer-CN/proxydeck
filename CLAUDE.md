...
## 当前状态（2026-09-12，v3.12.0 已发版）

- **v3.12.0 已发布**：WorkBuddy 双区账号池（6004 自动换号、按请求分摊轮询、
  ...
  提交链：`1140062`（tuanjie）→ `1531e0c`（codebuddy）→ `71f2ffd`（ui.html）→
  `d846fc0`（发版）→ `e02f015`（exe，干净树 @ d846fc0）
- **v3.11.0**：脱敏扩全角色（根治 11128 自我投毒）+ 历史版本日志全量改大白话；
  段位校正 3.10.8 → 3.11.0。v3.10.8：WorkBuddy 国际版接入（同甲板切换、
  出站角色降级 developer→system）。更早细节见 `CHANGELOG.md`
- 远程 main 与本地同步；仓库 github.com/Amer-CN/proxydeck（remote 名 `myrepo`，
  ...
  GitHub 会重定向，能用但名字陈旧）
- 服务现状（2026-09-12 15:4x 复核）：8785 / 8786 / 8787 / 8788 / 8789 / 8891 六个插件
  **全部在线**；GUI（PID 31996，15:4x 起）已是 v3.12.0 新包（发布产物烟测过：
  /v1/accounts 出 INTL 双号、rescan 正常）。
  **插件进程都早于本次换装时点**，Windows 下进程持有换装前的映像 → **后端仍跑旧包**
  （8787/8789 跑的是 v3.11.0 时点的包，全角色脱敏修复在线；账号池/注水多锚
  **不在运行中的后端里**）。
  ⚠ **拉杆不会换包**：`pluginStart` 探活到端口健康就「接管复用，不杀不重启」（见下条），
  想让后端换 v3.12.0 必须**熄火再点火**（拉回最左 → 再拉到最右）。
  **这次与以往不同：账号池是 8787/8789 的在途功能，不换包就不生效**——
  INTL 的 amer-cn 限流到 19:37:59，换包后该通道自动用 swch-cn 顶上，建议尽快对
  WorkBuddy 两区各熄火点火一次（其余四插件无在途功能，不急）。
