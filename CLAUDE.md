...
## 当前状态（2026-09-12，v3.12.1 已发版）

- **v3.12.1 已发布**：账号池区卡片化返工——复用团结账号矩阵的 mx-card 卡片语言
  （LED/徽章/元信息/操作 + N/M 可用计数），替换 v3.12.0 那版内联样式平铺列表
  （用户评价"好丑，为啥不复用"——根因是第二期简报只要求自包含插入、没指定复用
  矩阵样式）。交互与后端零改动，段位=修订号。提交链 `cea593a` → `5941d9e` →
  `4cce55e`（exe，干净树 @ 5941d9e）
- **v3.12.0**：WorkBuddy 双区账号池（6004 自动换号、按请求分摊轮询、
  /v1/accounts 管理端点、甲板账号池区、hy4 配置行前端隐藏——后端兜底保留，未来
  新免费模型限流稍改可接入）+ 注水检测两轮（官方基准重锚 + 多锚比对 +
  TokenRouter 远端锚点）。两批分别来自账号池会话与并行注水会话（后者用户确认
  完成后由发版会话代为入库，归属见提交消息）。
  v3.11.0：脱敏扩全角色（根治 11128 自我投毒）+ 日志全量改大白话；段位校正
  3.10.8 → 3.11.0。更早细节见 `CHANGELOG.md`
- 远程 main 与本地同步；仓库 github.com/Amer-CN/proxydeck（remote 名 `myrepo`，
  ...
  GitHub 会重定向，能用但名字陈旧）
- 服务现状（2026-09-12 16:2x 复核）：8785 / 8786 / 8787 / 8788 / 8789 / 8891 六个插件
  **全部在线**。用户 15:5x 手动熄火点火过 8787/8788/8789（拉杆换包，账号池与注水
  多锚已生效——实测真实 8789 /v1/accounts 返回 INTL 双号 swch-cn/amer-cn 均可用）；
  8785/8786/8891 仍是 09-11 的旧映像进程（无在途功能，不急）。GUI（PID 24988）
  已是 v3.12.1 新包——v3.12.1 是纯 UI 返工，**后端无需再换包**。
  ⚠ **拉杆不会换包**：`pluginStart` 探活到端口健康就「接管复用，不杀不重启」（见下条），
  想让后端换包必须**熄火再点火**（拉回最左 → 再拉到最右）。
  ⚠ 账号池冷却表是**进程内存态**：换包/重启后冷却记录清零，号显示"可用"，等再次
  撞 6004 才重新记冷却（上游 reset 时点不落盘，重启丢一次无害）。
...
- 下一版待办：① 账号池已知边界——区域拨杆键盘方向键切换不触发列表重拉（鼠标/触摸
  正常），用户裁决是否补；② hy4 键盘可达性同前。新坑/结论随时按惯例入册
